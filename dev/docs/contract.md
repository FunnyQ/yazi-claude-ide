...
- **I1.** A live selection is state, not a gesture. The editor MAY publish it as the selection changes, with no keypress, and the sidecar MUST NOT ask the editor for it. It writes to the single slot section D owns, and I7 and I8 are what let it do so without fighting D4 and D6. An editor that publishes nothing costs the user nothing: sections C, D, and H are unaffected.
