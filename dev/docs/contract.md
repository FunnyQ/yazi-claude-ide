...
- **I1.** Sending a range *mention* MUST be an explicit user gesture in the editor, bound to a key, as in H1. The editor's *live selection* is a different thing and MAY be published as it changes (I9). The sidecar MUST NOT ask the editor for either one. The two are not interchangeable: a mention accumulates in the prompt and survives submission, while a live selection is a single slot section D already owns — I11 and I12 are what let the editor write to that slot without fighting D4 and D6.
...
- **I5.** The sidecar MUST send `at_mentioned` as a JSON-RPC notification with `params` exactly `{filePath, lineStart, lineEnd}`. This is the one case H4's omission does not cover: an editor selection really does have a range, which is the whole reason this channel exists. The pair is 0-based and inclusive: measured 2026-08-09 against 2.1.226 through the whole path, a Neovim selection of lines 29-30 reached the wire as `28, 29` and rendered `@CHANGELOG.md#L29-30`. See [baseline.md](baseline.md).
...
- **I9.** The editor MAY publish its live selection under `claude-editor-selection`, which the sidecar MUST subscribe to alongside the kinds of I2, addressed exactly as I2 and I3 require, carrying the same `{yaziId, url, lineStart, lineEnd}` body with the same 1-based inclusive lines (I4) and dropped on the same grounds (I6). It is a second kind rather than a flag on the first because the two differ in every clause below: one is a gesture that accumulates, the other is a state that replaces. Publishing it is optional — a range mention is complete without it. The body MAY carry one field the mention has no use for, `text`, holding the selected contents; I10 is what needs it. An editor SHOULD omit `text` above a size of its own choosing, because selecting a whole file is one keystroke and the alternative is pushing that file through DDS and the WebSocket to drive a line count.
- **I10.** For each live selection the sidecar MUST push `selection_changed` shaped as D1 and D2 require, with `selection` carrying the converted 0-based range, `isEmpty: false`, and `text` set to whatever `text` the editor published — `""` when it published none. **This is the one push where `text` is not empty, and D2's reference to C2 does not reach it.** Measured 2026-08-09 against 2.1.226: a frame with the range filled in, `isEmpty: false`, and `text: ""` was accepted and drew `In CLAUDE.md` — the file chip, not a range — so the CLI computes its `N lines selected` display from the contents and not from `selection`. C4 is unchanged and still binding: the sidecar MUST NOT read a file to fill this field. The contents come from the editor, which already had them in a buffer, and only ever cover what the user selected by hand. **What `text` does is larger than the display it was added for**: measured the same day, the agent's context receives `The user selected the lines 7 to 14 from <path>:` followed by those lines verbatim, where an empty `text` produces only `The user opened the file <path> in the IDE.` Selecting in the editor therefore puts those lines in front of the agent with no submission and no mention. That is the promise this clause makes, and an editor that does not want to make it MUST omit `text` — I9 allows exactly that, at the cost of the line count.
- **I11.** A live selection MUST NOT be deduped, so D6 does not apply to it. Dragging a selection publishes range after range for one unchanged path, and a path-keyed dedupe would freeze the CLI on whichever range arrived first.
- **I12.** After pushing a live selection the sidecar MUST forget `last_pushed`, so the next yazi `hover` pushes even when it lands on the file the editor was just in. Without it, quitting the editor and moving around yazi leaves the CLI displaying a selection the user has already left.
- **I13.** With no connection open a live selection MUST be dropped and MUST NOT be queued (D7, I7). Unlike D3, no joining connection is owed it: the editor may well have exited by then, and a stale range is worse than none.
